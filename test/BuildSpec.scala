...
class BuildSpec extends AnyWordSpec with Matchers {
  ...
  "Modifying the content" should {
    "update all assets urls in manifest.css to start with the base route" in {
      ...
      updateFileContent(
        filePath,
        originalContent.replace("url(\"", "url(\"/guides/safety-and-security-import-service-guide")
      )
      ...
      updatedContent should include("url(\"/guides/safety-and-security-import-service-guide")
    }
  }
}
